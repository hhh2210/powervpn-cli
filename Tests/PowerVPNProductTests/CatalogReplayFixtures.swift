...
/// Recorded catalog-rejection shapes for deterministic offline replay.
///
/// Each entry is one server-behavior shape observed (or structurally
/// enumerated from the failure taxonomy) for the intermittent portal
/// catalog degradation: an accepted password login whose resource-list
/// reply then fails catalog mapping. The replay feeds these bytes through
/// the real acquisition pipeline — snapshot minting, `selectUnique`,
/// classification — so the recorded 1-in-8 intermittent rejection becomes
/// a deterministic regression test instead of a live-only event.
///
/// Value-free by construction: synthetic XML only, RFC 5737 / RFC 1918
/// documentation addresses, closed tokens, no intranet values.
...
/// time. Two taxonomy classes are unreachable end-to-end and are covered
/// by `ProductPortalDryRunCatalogFailureTests` at the classifier level:
/// `resource_list_duplicate` (rejected by the snapshot-descriptor gate at
/// mint time, before any lease exists) and `material_too_large` (the
/// bounded XML parser rejects oversized documents before mapping runs).
